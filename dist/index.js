...
const deployCommands = async () => {
    try {
        ...
        const deployedCommands = await rest.get(Routes.applicationCommands(process.env.CLIENT_ID));
        console.log(`[INFO] Deployed ${commands.length} commands: ${deployedCommands.map((cmd) => cmd.name).join(', ')}`);
    }
    catch (error) {
        console.error(`[ERROR] Error deploying commands: ${error}`);
    }
};
const { Client, GatewayIntentBits, Partials, Collection, ActivityType, PresenceUpdateStatus, Events } = require('discord.js');
...
client.on(Events.InteractionCreate, async (interaction) => {
    try {
        if (interaction.isChatInputCommand()) {
            ...
            console.log(`[INFO] Received command: ${interaction.commandName} from user ${interaction.user.tag}`);
            if (!command)
                return;
            ...
            console.log(`[INFO] Received modal submit: ${interaction.customId} from user ${interaction.user.tag}`);
